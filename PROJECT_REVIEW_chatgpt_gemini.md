...
本章只納入與 Money Trade 目前「日頻台股選股、整股交易、每日訊號、React 儀表板與 LINE 通知」直接相關、且投入報酬合理的項目。所有新增功能皆應遵守既有的 `research → shadow → production` 治理流程，不能直接影響正式策略。

## 34.1 P0：公司行動與雙價格序列（調整價／真實價）

### 目的

避免除權息、現金股利、配股、減資或權利分派，造成技術指標、報酬、ATR、停損與帳戶現金流被同一套價格錯誤處理。
...
| 場景 | 主要資料／邏輯 |
| --- | --- |
| 均線、動能、長期報酬、相對強弱 | `*_adj` 調整後價格序列 |
| ATR、日內波動與觸價規則 | 使用經公司行動處理後的連續序列；需在測試中明確驗證除權息日前後不產生假訊號 |
| 下單價格、持倉市值、交易金額 | `*_raw` 真實價格 |
| 漲跌停、成交可行性、跳空風險 | 真實價格、交易規則與公司行動事件 |
| 現金股利與配股 | 實際增加帳戶現金、調整持有股數；不可只靠調整後價格隱含處理 |

### 實作項目

1. 新增 `trading_code_ml/src/corporate_actions.py`，統一抓取、保存與套用公司行動。
2. 在 `data_fetcher.py` 輸出 raw 價格與公司行動事件，不覆寫原始資料。
3. 在 `feature_engine.py` 明確指定每個指標使用的價格欄位。
4. 在 `backtester.py` 加入 ex-date 的現金、持股數與成本調整邏輯。
5. 新增除權息測試樣本，驗證不會因價格斷點觸發假停損、假訊號或錯誤報酬。

### 驗收標準

- 同一檔股票在除權息前後的調整後報酬序列連續合理。
- 真實持倉現金、股數與市值可完整對帳。
- 除權息日不會因資料處理錯誤誤觸發 stop loss、trailing stop 或 replacement。

---

## 34.2 P1：Parquet 漸進式資料層與增量更新

### 原則

不立即刪除 CSV。CSV 保留作為原始資料與人工稽核格式；日常讀取、特徵合併、回測輸入與研究分析逐步改用 Parquet。
...
data/
  raw/
    prices/source=fubon_neo/year=YYYY/*.parquet
    institutional/source=twse_t86/year=YYYY/*.parquet
    revenue/source=finmind/year=YYYY/*.parquet
    corporate_actions/source=fubon_neo/year=YYYY/*.parquet
  processed/
    features/date=YYYY-MM-DD/*.parquet
    universe/date=YYYY-MM-DD/*.parquet
  marts/
    all_features_latest.parquet
    official_signal_snapshot.parquet
```

### 實作項目

1. 導入 `pyarrow`，建立共用 `storage.py`：負責 schema、寫入、讀取、壓縮與 atomic replace。
2. 新增 `data_schema.py`，固定行情、法人、營收、特徵與訊號的欄位型別。
3. `update_latest_features.py` 改為只重算受更新日期影響的區間，而不是每日重建全部歷史 CSV。
4. 輸出 `data_version.json`，記錄每個資料集的最新日期、筆數、schema version 與 hash。
5. 保留 `all_features.csv` 作為相容性輸出，待前端與其他腳本全部改讀 Parquet 後再評估淘汰。

### 驗收標準

- 同一份資料由 CSV 與 Parquet 讀入後列數、欄位、關鍵統計一致。
- 每日更新僅重算必要範圍。
- Grid search 與 walk-forward 不再因全量 CSV I/O 成為主要等待時間。

---

## 34.3 P1：日頻交易成本分析（TCA）與可成交性模型

### 目的

策略的大量交易來自 `replacement_switch`，交易成本與成交可行性會直接影響正式策略是否可實作。此階段先建立日頻 TCA，不依賴逐筆或五檔歷史資料。
...
illiquidity_slippage_bps
estimated_total_cost
actual_or_simulated_turnover
avg_daily_value_20d
participation_rate
limit_status
fill_status
...
estimated_slippage_bps =
    base_spread_bps
  + volatility_component_bps
  + participation_rate_component_bps
  + illiquidity_component_bps
  + event_risk_component_bps
```

其中：

- `base_spread_bps`：基本買賣價差假設。
- `volatility_component_bps`：以 ATR%、近 20 日波動度或當日振幅衡量。
- `participation_rate_component_bps`：依委託金額占當日或近 20 日平均成交額的比例提高成本。
- `illiquidity_component_bps`：針對低成交額、低周轉、價格跳動較大的股票加碼。
- `event_risk_component_bps`：漲跌停、跳空、停牌恢復等特殊情況採保守價格或部分成交。
...
| 模式 | 用途 |
| --- | --- |
| `ideal_fill` | 比較訊號本身，不作為正式績效。 |
| `realistic_fill` | 納入手續費、證交稅、日頻滑價、參與率與部分成交。正式前端預設使用此結果。 |
| `stress_fill` | 對跳空、低流動性、漲跌停與高波動採更保守假設，用於風險壓力測試。 |

### Replacement 專項分析

新增 `replacement_analysis.csv`，每次回測至少包含：

- replacement 次數、毛利、淨利與總成本。
- 替換前後分數差與排名差。
- 被替換股票若繼續持有 5／10／20／60 日的反事實報酬。
- 替換後股票相對被替換股票的超額報酬。
- 各 regime 下 replacement 成功率與成本。
- 年化換手率、交易成本占毛利比例、平均持有天數。

### 驗收標準

- 每筆 trade log 都能解釋成本來源與成交狀態。
- `replacement_switch` 的淨貢獻在扣除成本後仍可獨立檢驗。
- 前端可同時顯示毛績效、淨績效與交易成本侵蝕比例。

---

## 34.4 P1：部位、ATR 停損與整股限制試算器

### 目的

讓策略目標部位、使用者真實持倉與整股交易限制使用同一套公式，避免前端試算、回測與每日訊號各自使用不同風控邏輯。
...
```text
frontend/src/components/RealPortfolioPanel.jsx
```

後端或 Python 核心需提供共用計算邏輯；前端不可自行複製一套風控公式。

### 輸入

- 總資金與可用現金。
- 股票代號、參考價格與預計進場價格。
- ATR、初始停損 ATR 倍數、實際停損價格。
- 單筆最大風險比例。
- 單股最大權重。
- 最大持股數。
- 最小交易單位。
...
- 最壞情境損失、含預估成本損失。
- 實際下單因整股限制造成的風險偏差。
- 是否超過單股、產業、相關性或成交量參與率限制。

### 建議核心公式

```text
per_share_risk = abs(entry_price - stop_price) + estimated_cost_per_share
risk_budget = portfolio_equity * max_risk_per_trade
shares_by_risk = floor(risk_budget / per_share_risk)
shares_by_cash = floor(available_cash / estimated_entry_cost_per_share)
shares_by_position = floor((portfolio_equity * max_position_pct) / estimated_entry_cost_per_share)
shares_by_liquidity = floor(max_participation_value / estimated_entry_cost_per_share)
final_shares = round_down_to_lot(min(all constraints), min_trade_unit)
```

### 驗收標準

- 前端試算結果與 Python `RiskManager.position_size()` 對同一輸入完全一致。
- 1000 股整股限制下，顯示「理論股數」與「實際可下單股數」的差異。
- LINE 建議下單量直接使用同一份結果。

---

## 34.5 P1：可重跑、可重試的任務化每日流程

### 目的

在不立即導入大型 DAG 平台的前提下，先讓每日流程具備清楚的依賴、重試、續跑、狀態記錄與發布控制。
...
generate_forward_simulation
generate_signals
run_risk_and_tca_checks
publish_sqlite
publish_frontend_data
build_frontend
final_health_gate
...
```text
runs/YYYY-MM-DD/<run_id>/task_status.json
```
...
- API 暫時失敗時，只重試失敗 task；不重跑已成功且輸入未改變的 task。
- 若中斷，可由最後失敗 task 續跑。
- 只有 `final_health_gate=PASS` 時，才允許發布 official artifact 與發送 LINE。
- `send_line_notification` 必須以 `run_id` 保證冪等；同一 run 不得重複推播。
- 先寫入 staging，再以 atomic publish 切換正式輸出，避免前端讀到半成品。

### 未來擴充

當每日流程已穩定 task 化、但需要 Web UI、多流程排程或跨主機執行時，再評估導入 Prefect。現階段不需先導入 Airflow 或 Dagster。

---

## 34.6 P1：DSR、多重測試控制與實驗註冊表

### 目的

大量 grid search、權重測試、出場測試、OOS 版本與防守版本，容易讓最高 Sharpe 或最高 CAGR 只是多次測試後的偶然最佳值。正式升版必須納入多重測試與統計穩健性判斷。
...
```text
research/experiment_registry.parquet
research/experiment_registry.csv
```
...
- Net-of-cost CAGR、Sharpe、最大回撤與年化換手率。
- Probabilistic Sharpe Ratio（PSR）。
- Deflated Sharpe Ratio（DSR），將多重嘗試與非正態報酬風險納入考量。
- 固定測試期與前瞻 shadow 期的結果。
- 各年度、各 regime 與壓力期間的穩定性。
- 對交易成本、滑價與參數的小幅變動是否敏感。

### 升版規則

一個策略不得僅因全期間 CAGR 或單一 OOS 表現較高而升版。至少需要：

1. 完整記錄實驗數量與假說。
2. 使用 production 成交成本模型。
3. DSR／PSR 通過內部設定門檻。
4. 在固定測試期與 shadow 期均不劣於現行策略的核心風險指標。
5. 能清楚解釋新增績效來自何種可交易的機制，而非更高換手或更樂觀成交假設。

---

## 34.7 P2：Docker 化與可攜式執行環境

### 目的

讓研究、每日正式流程、前端 build 與 Cloud Run 部署在不同電腦或雲端環境時使用可重現依賴，降低 Python 套件、Node 版本與系統路徑造成的執行差異。
...
- 集中設定與相對路徑。
- `.env.example`、secret 管理與 `.gitignore`。
- data volume 掛載規則。
- Fubon SDK、憑證與登入流程在容器內的可行性驗證。

### 建議容器切分

```text
services/
  quant-pipeline/     # Python 資料、特徵、回測、通知流程
  frontend-builder/   # React / Vite build
  cloud-line/         # Flask Cloud Run service
```

### 原則

- 不將 token、憑證或真實 `.env` 放進 image。
- raw data、processed data、results 與 SQLite 透過 volume 或雲端儲存掛載。
- image tag 使用 git commit 或 release version。
- 每次 official run 在 manifest 中記錄 image version。
...
1. 公司行動與 raw／adjusted 價格序列設計。
2. 日頻 TCA、replacement 成本拆解與可成交性檢查。
3. 部位／ATR／整股限制試算器，並與 `RiskManager` 共用邏輯。
4. 任務化每日流程、單 task 重試、staging publish 與 LINE 冪等控制。

### 第二優先：流程穩定後

1. Parquet 資料層與增量特徵更新。
2. 實驗註冊表、PSR、DSR 與策略升版條件。
3. Docker 化 Python 正式流程與前端 build。

### 第三優先：資料與交易規模成熟後再評估

1. 引入 Prefect 作為排程與監控平台。
2. 取得逐筆或五檔歷史資料後，進一步做 order-book 級 TCA。
3. 在使用 ML ranking／分類模型時，再評估 CPCV、purging 與 embargo 的進階驗證。

## 36. 本章完成後的預期成果

完成本章內容後，Money Trade 應具備：

- 正確處理台股除權息與公司行動的價格、持倉與現金帳。
- 可用日頻成本模型量化 replacement 的真正淨價值。
- 回測、前端、LINE 與真實持倉使用相同的部位風控邏輯。
- 每日流程可局部重試、可安全重跑、不可重複通知。
- 不再只依歷史最高 Sharpe 或 CAGR 選策略，而能將多重測試、成本與前瞻驗證納入治理。
- 在不過早引入大型基礎設施的前提下，保留未來容器化、排程化與高精度 TCA 的升級路徑。
